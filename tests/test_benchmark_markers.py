...
def benchmark_db(tmp_path_factory: pytest.TempPathFactory) -> str:
    db_path = str(tmp_path_factory.mktemp("data") / "benchmark.db")
...
def test_marker_benchmark(benchmark_db: str) -> None:
    matcher = AggregatedLicenseMatcher(benchmark_db)

    print("\n\n" + "=" * 60)
    print(f"{'FIXTURE':<20} | {'WITHOUT MARKERS':<15} | {'WITH MARKERS':<15}")
    print("-" * 60)
    ...
    for name, data in FIXTURES.items():
        ...
        top_no: str = res_no[0]["license_id"] if res_no else "None"
        score_no: float = res_no[0]["score"] if res_no else 0.0

        # With markers
        res_yes = matcher.match({"text": text, "enable_markers": True})
        top_yes: str = res_yes[0]["license_id"] if res_yes else "None"
        score_yes: float = res_yes[0]["score"] if res_yes else 0.0
        method: str = res_yes[0].get("method", "unknown") if res_yes else "N/A"
